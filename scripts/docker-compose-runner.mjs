...
if (!existsSync(dockerConfigFile)) {
  writeFileSync(dockerConfigFile, JSON.stringify({ auths: {} }, null, 2) + '\n');
}
...
if (composeArgs.length === 0) {
  console.error('Usage: bun run scripts/docker-compose-runner.mjs <docker compose args>');
  process.exit(1);
}
...
child.on('error', (error) => {
  if (error.code === 'ENOENT') {
    console.error('Docker CLI not found in PATH. Please install Docker Desktop/Engine and retry.');
  } else {
